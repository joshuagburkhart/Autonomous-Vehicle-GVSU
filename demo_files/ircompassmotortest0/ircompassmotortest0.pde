...
*@note    - This code is intended to be used to demonstrate
...
static int  RIGHT          = 0,
            ...
            AHEAD          = 0,
            BACK           = 1,
            EAST           = 190,//180, //ideal degrees commented out, tuned degrees used
            SOUTHEAST      = 218,//225,
            SOUTH          = 256,//270,
            SOUTHWEST      = 274,//315,
            WEST           = 19,//0,
            ...
            CMP_TOLERANCE  = 5,  //determines the compass tolerance
...
void setup(){
  ...
  headingArray[0]  = SOUTH;
  headingArray[1]  = EAST;
  headingArray[2]  = NORTH;
  headingArray[3]  = WEST;
...
void loop(){
  int    k,j,o;
  double dir;
  for(k = 0; k < 4; k++){                        //go through the lat and long arrays
    ...
    //for(o = 0; o < 5; o++){//just go in one direction for a while
      while(getIrReading(10,10) < IR_TOLERANCE){  //while rover is not too close to an object
        ...
        turnToHeading(headingArray[k]);//dir);                       //turn the rover from the current heading to dir heading
